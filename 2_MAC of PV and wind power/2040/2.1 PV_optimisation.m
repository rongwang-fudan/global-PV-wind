...
for iiaa = 1:size(country_needadjust,1)
    ...
    [m22,n]=find(powerunit_country_IX_IX==country & optpowerunit_IX_all(:,35)==1 & unitmin_all<=4);
    if ~isempty(m22)
        ...
        if ~isempty(optpowerunit)
            ...
            if size(optpowerunit_IX,1)>1
                ...
                if zerotracker_year_country(country)==2050
                    for p1=0:1:100
                        ...
                        for p2=0:1:(100-p1)
                            pene(2,1)=(p1+p2)/100; % peneratration ratio in 2030-2040
                            pene(3,1)= 1; % peneratration ratio in 2050-2060
                            costall_PV=0;
                            costall_onshorewind=0;
                            costall_offshorewind=0;
                            cumulativecapacity=0;
            ...
            if size(optpowerunit_IX,1)==1
                ...
                %             filename = strcat('H:\global-PV-wind\ANS\3unitmin_region',num2str(country),'_county0811_all_pv_pro.mat');
...
save('H:\global-PV-wind\ANS\unit_PV_global_2040.mat','unit_PV_global');
