...
load('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\tranmission_lines_all.dat','-mat');  % lines_IX
...
% load('H:\world\code\数据处理\unitid_lcoe_offshorewind1227_2_xz_100GW.mat');
...
load('H:\global-PV-wind\ANS\index_mineral_pv_time2_county0811_all_cou_5%_LRglobal_inilow_pro.mat') % 按照成本排序&考虑建厂时间后保留的PV电厂原始序号
load('H:\global-PV-wind\ANS\index_mineral_ons_time2_county0811_all_cou_5%_LRglobal_inilow_pro.mat') % 按照成本排序&考虑建厂时间后保留的onshorewind电厂原始序号
load('H:\global-PV-wind\ANS\index_mineral_off_time2_county0811_all_cou_5%_LRglobal_inilow_pro.mat') % 按照成本排序&考虑建厂时间后保留的offshorewind电厂原始序号
...
% [m,n]=find(line_IX_all(num_lines+1:end,11)==35);
% line_IX_all_cn = line_IX_all(num_lines+m,:);
% 
% for j=1:size(line_IX_all_cn,1)
%     if line_IX_all_cn(j,11)==35 
%         line_IX_all_cn(j,12)=GADM_pro120(line_IX_all_cn(j,3),line_IX_all_cn(j,4)); % 终点pro
%         [mm1,nn1]=find(pro_CN_reg(:,1)==line_IX_all_cn(j,10));
%         if ~isempty(mm1)
%         line_IX_all_cn(j,13)=pro_CN_reg(mm1,2); % 起点region(1-4,1-7)
%         end
%         [mm1,nn1]=find(pro_CN_reg(:,1)==line_IX_all_cn(j,12));
%         if ~isempty(mm1)
%         line_IX_all_cn(j,14)=pro_CN_reg(mm1,2); % 终点region(1-4,1-7)
%         end
%     end
% end
% [m,n]=find(line_IX_all(num_lines+1:end,11)==35);
% optpowerunit_IX_cn = optpowerunit_IX(m,:);
...
%%
% load('H:\world\code\P2070_others.mat'); % Power_other-renewable_xz.xlsx, TWh/year
% P2019_others = sum(P2070_others,2);
% 1.Biomass; 2.Geothermal; 3.Hydro; 4.Nuclear; 5.Ocean; 6.CSP
...
save('H:\global-PV-wind\ANS\powerdemand_monhour2070_ele288_country.mat','powerdemand_monhour2070_ele288_country')  %  % TWh/h 
...
%
% load('H:\world\code\ID_pro3.mat') % pro FID 	FIRST_ID_0  ID_country120_0214	FIRST_ID_1
...
% load('H:\world\code\utilize_ratio2060_UHV_storage_inter_county_all.mat')
...
save('H:\global-PV-wind\ANS\utilize_ratio2060_county_all_pro.mat','utilize_ratio2060')
save('H:\global-PV-wind\ANS\utilize_ratio2060_UHV_county_all_pro.mat','utilize_ratio2060_UHV')
save('H:\global-PV-wind\ANS\utilize_ratio2060_UHV_storage_county_all_pro.mat','utilize_ratio2060_UHV_storage')
save('H:\global-PV-wind\ANS\utilize_ratio2060_UHV_storage_reg_county_all_pro.mat','utilize_ratio2060_UHV_storage_reg')
save('H:\global-PV-wind\ANS\utilize_ratio2060_UHV_storage_inter_county_all_pro.mat','utilize_ratio2060_UHV_storage_inter')

save('H:\global-PV-wind\ANS\storage_max_plant_UHV_storage_county_all_pro.mat','storage_max_plant_UHV_storage')   % TWh/h
save('H:\global-PV-wind\ANS\storage_year_plant_UHV_storage_county_all_pro.mat','storage_year_plant_UHV_storage')  % TWh/year
save('H:\global-PV-wind\ANS\storage_max_plant_UHV_storage_reg_county_all_pro.mat','storage_max_plant_UHV_storage_reg')   % TWh/h
save('H:\global-PV-wind\ANS\storage_year_plant_UHV_storage_reg_county_all_pro.mat','storage_year_plant_UHV_storage_reg')  % TWh/year
save('H:\global-PV-wind\ANS\storage_max_plant_UHV_storage_inter_county_all_pro.mat','storage_max_plant_UHV_storage_inter')   % TWh/h
save('H:\global-PV-wind\ANS\storage_year_plant_UHV_storage_inter_county_all_pro.mat','storage_inter_year_plant_UHV_storage')  % TWh/year
