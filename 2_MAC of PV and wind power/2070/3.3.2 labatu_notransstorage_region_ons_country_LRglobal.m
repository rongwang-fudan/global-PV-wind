...
%% mineral limittation
load('H:\Global PV and wind\ANS\index_mineral_pv_county0811_2_all_CNfirst_5%_inilow_pro.mat') % 
load('H:\Global PV and wind\ANS\index_mineral_ons_county0811_2_all_CNfirst_5%_inilow_pro.mat') % 
load('H:\Global PV and wind\ANS\index_mineral_off_county0811_2_all_CNfirst_5%_inilow_pro.mat') % 
    ...
    if ~isempty(optpowerunit)
        ...
        if size(optpowerunit_IX,1)>1
                ...
                for p2=0:1:(100-p1)
                    ...
                    for p3=0:1:(100-p1-p2)
                        ...
                        for p4=0:1:(100-p1-p2-p3)
                            ...
                            pene(5,1)= 1; % peneratration ratio in 2060-2070
                            idx=find(optpowerunit_IX(:,36)>=pene(4,1)); % percentage of energy
                            unit2(4,1)=idx(1);
                            unit2(5,1)=size(optpowerunit_IX,1);
                            costall_PV=0;
                            ...
                            for yy=1:5
                                if unit2(yy,1)>=unit1 && unit2(yy,1)~=1
                                    ...
                                    pene_real(yy,2)=pe_ratio(unit2(yy,1),1); % 
                                    unit1=unit2(yy,1)+1;
                                end
                            end
            ...
            for p1=0:0.5:peneminA(1,1)
                ...
                costmax1 = 0;
                pene=zeros(10,2);
                pene(1,1)=p1/100; % peneratration ratio 
                idx=find(optpowerunit_IX(:,36)>=pene(1,1)); % percentage of energy
                unit2(1,1)=idx(1);
                for p2=peneminA(1,1)-p1
                    pene(2,1)=(p1+p2)/100; % peneratration ratio
                    idx=find(optpowerunit_IX(:,36)>=pene(2,1)); % percentage of energy
                    unit2(2,1)=idx(1);
                    for p3=peneminA(1,1)-p1-p2:1:peneminA(2,1)-p1-p2
                        pene(3,1)=(p1+p2+p3)/100; % peneratration ratio 
                        idx=find(optpowerunit_IX(:,36)>=pene(3,1)); % percentage of energy
                        unit2(3,1)=idx(1);
                        for p4=peneminA(2,1)-p1-p2-p3
                            pene(4,1)=(p1+p2+p3+p4)/100; % peneratration ratio 
                            idx=find(optpowerunit_IX(:,36)>=pene(4,1)); % percentage of energy
                            unit2(4,1)=idx(1);
                            for p5=peneminA(2,1)-p1-p2-p3-p4:1:peneminA(3,1)-p1-p2-p3-p4
                                pene(5,1)=(p1+p2+p3+p4+p5)/100; % peneratration ratio 
                                idx=find(optpowerunit_IX(:,36)>=pene(5,1)); % percentage of energy
                                unit2(5,1)=idx(1);
                                for p6=peneminA(3,1)-p1-p2-p3-p4-p5
                                    pene(6,1)=(p1+p2+p3+p4+p5+p6)/100; % peneratration ratio
                                    ...
                                    for p7=peneminA(3,1)-p1-p2-p3-p4-p5-p6:1:peneminA(4,1)-p1-p2-p3-p4-p5-p6
                                        pene(7,1)=(p1+p2+p3+p4+p5+p6+p7)/100; % peneratration ratio
                                        idx=find(optpowerunit_IX(:,36)>=pene(7,1)); % percentage of energy
                                        unit2(7,1)=idx(1);
                                        for p8=peneminA(4,1)-p1-p2-p3-p4-p5-p6-p7
                                            pene(8,1)=(p1+p2+p3+p4+p5+p6+p7+p8)/100; % peneratration ratio 
                                            ...
                                            for p9=peneminA(4,1)-p1-p2-p3-p4-p5-p6-p7-p8:1:peneminA(5,1)-p1-p2-p3-p4-p5-p6-p7-p8
                                                pene(9,1)=(p1+p2+p3+p4+p5+p6+p7+p8+p9)/100; % peneratration ratio
                                                pene(10,1)= 1; % peneratration ratio
        ...
        for i = 1:size(powerunit_IX,1)
            ...
            if optpowerunit_IX(aa,35) ==2
                unit_onshorewind(bb,1)=i;% 
            end
            if optpowerunit_IX(aa,35) ==3
                unit_offshorewind(bb,1)=i;% 
            end
            %     i
        end
...
save('H:\Global PV and wind\ANS\3power_country_region_county0811_all_country_5%_LRglobal_inilow_ons_pro2070.mat','power_country');
save('H:\Global PV and wind\ANS\3unit_onshorewind_region_county0811_all_country_5%_LRglobal_inilow_ons_pro2070.mat','unit_onshorewind_global');
save('H:\Global PV and wind\ANS\learnprice_region_county0811_all_country_5%_LRglobal_inilow_ons_pro2070.mat','learnprice_region'); 
save('H:\Global PV and wind\ANS\cost_all2020_cou_ons_pro2070.mat','cost_all2020_cou');
save('H:\Global PV and wind\ANS\cost_ave_cou_ons_pro2070.mat','cost_ave_cou');
save('H:\Global PV and wind\ANS\costmin_cou_ons_pro2070.mat','costmin_cou');
