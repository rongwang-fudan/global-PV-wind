...
% load('H:\Global PV and wind\ANS\数据处理\unitid_lcoe_offshorewind1227_2_xz_100GW.mat');
...
load('H:\Global PV and wind\ANS\Country_ID_withUHV.mat')  % 有UHV Sation所在的国家ID
...
load('H:\Global PV and wind\Data\pro_CN_reg.mat') % 第一列是pro ID，第二列是对应中国国内的region ID (1-7)
...
clear position_c
save('H:\Global PV and wind\ANS\distance_CN_8_2070.mat','distance_CN') % km
%%
% load('H:\Global PV and wind\ANS\P2070_others.mat'); % TWh/year
...
sum(sum(powerdemand_monhour(:,m),2))
sum(sum(powerdemand_monhour2070_REG(:,1:583)))
sum(sum(powerdemand_monhour2070_REG(:,1:583)))-sum(sum(powerdemand_monhour(:,m),2))
...
%
% load('H:\Global PV and wind\ANS\ID_pro3.mat') % pro FID 	FIRST_ID_0  ID_country120_0214	FIRST_ID_1
...
load('H:\Global PV and wind\Data\region_connect0811.mat')  % 1 起点；2 终点。
region_connect = region_connect0811;
clear region_connect0811
% load('H:\Global PV and wind\ANS\utilize_ratio2060_UHV_storage_inter_county_all.mat')
% optpowerunit_IX_IX(:,1)=optpowerunit_IX_IX(:,1).*utilize_ratio2060_UHV_storage_inter;
load('H:\Global PV and wind\ANS\distance_UHV_Station_all.mat')  % 千米
...
linescapacity_plant=zeros(size(distance_UHV_Station,1),size(distance_UHV_Station,1),1000);
    ...
    if country2==-1 % 没有UHV station
        ...
        for zz = 1:288
            ...
            if id_cou(country,zz)==1 && id_cou(country,289)==0
                ...
                a = min(min([powergenerat_monhour(zz,i)*ef1*ef3(cho(i))+sum(P_country(:,country));sum(powerdemand_monhour(:,country))]));
                if a==sum(sum(powerdemand_monhour(:,country)))
                    %                 id_cou(country,289)=1;
                    id_cou(country,:)=1;
                end
                powergenerat_monhour_zz1 = powergenerat_monhour(zz,i);
                b = (a-P_pro_1)/ef1/ef3(cho(i));
                b(b<0)=0;
                powergenerat_monhour(zz,i) = powergenerat_monhour(zz,i)-b; % i电厂剩余可用电量
                P_ef1 = (powergenerat_monhour_zz1-powergenerat_monhour(zz,i))*ef1*ef3(cho(i));
                ...
                etrans(region,region) = etrans(region,region) + P_ef1/ef1/ef3(cho(i));
                etrans_type(region,region,type(i,1)) = etrans_type(region,region,type(i,1)) + P_ef1/ef1/ef3(cho(i));
                etrans_cou(country,country) = etrans_cou(country,country) + P_ef1/ef1/ef3(cho(i));
                etrans_cou_type(country,country,type(i,1)) = etrans_cou_type(country,country,type(i,1)) + P_ef1/ef1/ef3(cho(i));
                etrans_REG(REG,REG) = etrans_REG(REG,REG) + P_ef1/ef1/ef3(cho(i));
                etrans_REG_type(REG,REG,type(i,1)) = etrans_REG_type(REG,REG,type(i,1)) + P_ef1/ef1/ef3(cho(i));
...
for i = 1:1:numpowerunit
    if rem(i,1000)==1 && floor(i/1000)~=0
        filename = strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str((i-1)/1000),'_battery_mechanical_pro1_8_2070.mat');
        save(filename, 'linescapacity_plant','-v7.3');
        linescapacity_plant=zeros(size(distance_UHV_Station,1),size(distance_UHV_Station,1),1000);
        nnnn = floor(i/1000);
    end
    i2=i;
                    ...
                    if powergenerat_monhour(zz,i)>0
                        ...
                        if dis2<=10000 & (id_REG(REG2,zz)+id_REG(REG2,289))==0
                            ...
                            if P_ef1*10^6>=1
                                ...
                                linescap1 = linescapacity_plant(REG,REG2,i2-nnnn*1000);
                                linescapacity_plant(REG,REG2,i2-nnnn*1000)=max([linescapacity_plant(REG,REG2,i2-nnnn*1000); P_ef1/ef2*10^6]); % transmission capacity MW
                                linescapacity(REG,REG2)=linescapacity(REG,REG2)+linescapacity_plant(REG,REG2,i2-nnnn*1000)-linescap1; % transmission capacity MW
                ...
                for iib = 1:size(REG2_2,2)
                    ...
                    if powergenerat_monhour(zz,i)>0
                        ...
                        if P_ef1*10^6>=1
                            ...
                            P_ef1 = (powergenerat_monhour_zz1-powergenerat_monhour(zz,i))*ef1*ef2;
                            if ~isempty(idxxx)
                                for iias = 1:size(idxxx,2)
                                    linescap1 = linescapacity_plant_CN(idxxx(iias),i2);
                                    linescapacity_plant_CN(idxxx(iias),i2)=max([linescapacity_plant_CN(idxxx(iias),i2); P_ef1/ef2*10^6]); % transmission capacity MW
                                    linescapacity_CN(idxxx(iias),1)=linescapacity_CN(idxxx(iias),1)+linescapacity_plant_CN(idxxx(iias),i2)-linescap1; % transmission capacity MW
                                end
                            end
                            P_REG(zz,REG2) = P_REG(zz,REG2) + P_ef1;
                            P_country1(zz,i) = P_country1(zz,i) + P_ef1;
...
filename = strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str(ceil(i/1000)),'_battery_mechanical_pro1_8_2070.mat');
save(filename, 'linescapacity_plant','-v7.3');
2
% domestic storage
nnnn = 0;
load('H:\Global PV and wind\ANS\linescapacity_plant1_battery_mechanical_pro1_8_2070.mat')% million $
for i = 1: 1:numpowerunit
    if rem(i,1000)==1 && floor(i/1000)~=0
        filename = strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str((i-1)/1000),'_battery_mechanical_pro1_8_2070.mat');
        save(filename, 'linescapacity_plant','-v7.3');
        filename = strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str((i-1)/1000+1),'_battery_mechanical_pro1_8_2070.mat');
        load(filename);
        nnnn = floor(i/1000);
    end
    i2=i;
                ...
                for ii2 = 1:1:size(IXXX,1)
                    if powergenerat_monhour(zz,i)>0
                        ...
                        if dis2<=10000 & id_REG(REG2,289)==0
                            powergenerat_monhour_zz1 = powergenerat_monhour(zz,i);
                            a = min(min([powergenerat_monhour(zz,i)*ef1*ef2*ef3(cho(i))+sum(P_REG(:,REG2));sum(powerdemand_monhour2070_REG(:,REG2))]));
                            ...
                            b = (a-P_pro_1)/(ef1*ef2*ef3(cho(i)));
                            b(b<0)=0;
                            P_ef1 = b*ef1*ef2*ef3(cho(i));
                            
                            if P_ef1*10^6>=1
                                powergenerat_monhour(zz,i) = powergenerat_monhour(zz,i)-b; % i电厂剩余可用电量
                                P_ef1 = (powergenerat_monhour_zz1-powergenerat_monhour(zz,i))*ef1*ef2*ef3(cho(i));
                                ...
                                etrans(region,region) = etrans(region,region) + P_ef1/ef1/ef2/ef3(cho(i));
                                etrans_type(region,region,type(i,1)) = etrans_type(region,region,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i));
                                etrans_cou(country,country) = etrans_cou(country,country) + P_ef1/ef1/ef2/ef3(cho(i));
                                etrans_cou_type(country,country,type(i,1)) = etrans_cou_type(country,country,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i));
                                etrans_REG(REG,REG2) = etrans_REG(REG,REG2) + P_ef1/ef1/ef2/ef3(cho(i));
                                etrans_REG_type(REG,REG2,type(i,1)) = etrans_REG_type(REG,REG2,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i));
...
for i = 1:1:numpowerunit
    if rem(i,1000)==1 && floor(i/1000)~=0
        filename = strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str((i-1)/1000),'_battery_mechanical_pro_8_2070.mat');
        save(filename, 'linescapacity_plant','-v7.3');
        load(strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str((i-1)/1000+1),'_battery_mechanical_pro1_8_2070.mat'))% million $
        nnnn = floor(i/1000);
    end
                    ...
                    for ii2 = 1:1:size(IXXX,1)
                        if powergenerat_monhour(zz,i)>0
                            ...
                            if dis2<=10000 && id_cou(cou2,289)==0 && id_cou(cou2,zz)==1
                                powergenerat_monhour_zz1 = powergenerat_monhour(zz,i);
                                a = min(min([powergenerat_monhour(zz,i)*ef1*ef2*ef3(cho(i))*ef4+sum(sum(P_country(:,cou2)));sum(sum(powerdemand_monhour(:,cou2)))]));
                                ...
                                b = (a-P_country_1)/(ef1*ef2*ef3(cho(i))*ef4);
                                b(b<0)=0;
                                P_ef1 = b*ef1*ef2*ef3(cho(i))*ef4;
                                
                                if P_ef1*10^6>=1
                                    powergenerat_monhour(zz,i) = powergenerat_monhour(zz,i)-b; % i电厂剩余可用电量
                                    P_ef1 = (powergenerat_monhour_zz1-powergenerat_monhour(zz,i))*ef1*ef2*ef3(cho(i))*ef4;
                                    linescap1 = linescapacity_plant(REG,REG2,i2-nnnn*1000);
                                    linescapacity_plant(REG,REG2,i2-nnnn*1000)=max([linescapacity_plant(REG,REG2,i2-nnnn*1000); P_ef1/ef2*10^6]); % transmission capacity MW
                                    linescapacity(REG,REG2)=linescapacity(REG,REG2)+linescapacity_plant(REG,REG2,i2-nnnn*1000)-linescap1; % transmission capacity MW
                                    storage_UHV_storage_regional(zz,i) = storage_UHV_storage_regional(zz,i) + powergenerat_monhour_zz1-powergenerat_monhour(zz,i);
                                    ...
                                    etrans(region,region) = etrans(region,region) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                    etrans_type(region,region,type(i,1)) = etrans_type(region,region,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                    etrans_cou(country,cou2) = etrans_cou(country,cou2) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                    etrans_cou_type(country,cou2,type(i,1)) = etrans_cou_type(country,cou2,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                    etrans_REG(REG,REG2) = etrans_REG(REG,REG2) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                    etrans_REG_type(REG,REG2,type(i,1)) = etrans_REG_type(REG,REG2,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
...
%                     if  sum(sum(P_region(:,reggg))) < sum(sum(powerdemand_monhour_region(:,reggg)))
                        ...
                        for ii2 = 1:1:size(IXXX,1)
                            if powergenerat_monhour(zz,i)>0
                                ...
                                if dis2<=10000 && id_cou(cou2,289)==0 && id_cou(cou2,zz)==1
                                    powergenerat_monhour_zz1 = powergenerat_monhour(zz,i);
                                    a = min(min([powergenerat_monhour(zz,i)*ef1*ef2*ef3(cho(i))*ef4+P_country_1;sum(sum(powerdemand_monhour(:,cou2)))]));
                                    if a==sum(sum(powerdemand_monhour(:,cou2)))
                                        %                 id_cou(cou2,289)=1;
                                        %                 [m,n]=find(floor(UHV_Station_country(1:size(UHV_Station_country,1),4))==cou2);
                                        %                 id_REG(m,289)=1;
                                    ...
                                    if P_ef1*10^6>=1
                                        ...
                                        etrans(region,reg2) = etrans(region,reg2) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                        etrans_type(region,reg2,type(i,1)) = etrans_type(region,reg2,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                        etrans_cou(country,cou2) = etrans_cou(country,cou2) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                        etrans_cou_type(country,cou2,type(i,1)) = etrans_cou_type(country,cou2,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                        etrans_REG(REG,REG2) = etrans_REG(REG,REG2) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
                                        etrans_REG_type(REG,REG2,type(i,1)) = etrans_REG_type(REG,REG2,type(i,1)) + P_ef1/ef1/ef2/ef3(cho(i))/ef4;
...
toc
filename = strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str(ceil(i/1000)),'_battery_mechanical_pro_8_2070.mat');
save(filename, 'linescapacity_plant','-v7.3');
save('H:\Global PV and wind\ANS\linescapacity_plant_battery_mechanical_CN_pro_8_2070.mat','linescapacity_plant_CN','-v7.3')% million $
etrans = etrans/288*8760;
etrans_type = etrans_type/288*8760;
etrans_cou = etrans_cou/288*8760;
etrans_cou_type = etrans_cou_type/288*8760;
etrans_REG = etrans_REG/288*8760;
etrans_REG_type = etrans_REG_type/288*8760;

etrans1 = etrans1/288*8760;
etrans_type1 = etrans_type1/288*8760;
etrans_cou1 = etrans_cou1/288*8760;
etrans_cou_type1 = etrans_cou_type1/288*8760;
etrans_REG1 = etrans_REG1/288*8760;
etrans_REG_type1 = etrans_REG_type1/288*8760;

save('H:\Global PV and wind\ANS\etrans_all_battery_mechanical_pro2_8_2070.mat','etrans1')% 实际有效发电量, TWh/year
save('H:\Global PV and wind\ANS\etrans_type_all_battery_mechanical_pro2_8_2070.mat','etrans_type1')%
save('H:\Global PV and wind\ANS\etrans_cou_all_battery_mechanical_pro2_8_2070.mat','etrans_cou1')%
save('H:\Global PV and wind\ANS\etrans_cou_type_all_battery_mechanical_pro2_8_2070.mat','etrans_cou_type1')%
save('H:\Global PV and wind\ANS\etrans_REG_all_battery_mechanical_pro2_8_2070.mat','etrans_REG1')%
save('H:\Global PV and wind\ANS\etrans_REG_type_all_battery_mechanical_pro2_8_2070.mat','etrans_REG_type1')%

save('H:\Global PV and wind\ANS\etrans_all_battery_mechanical_withoutPUE_pro2_8_2070.mat','etrans')% 不考虑用电效率的发电量, TWh/year
save('H:\Global PV and wind\ANS\etrans_type_all_battery_mechanical_withoutPUE_pro2_8_2070.mat','etrans_type')%
save('H:\Global PV and wind\ANS\etrans_cou_all_battery_mechanical_withoutPUE_pro2_8_2070.mat','etrans_cou')%
save('H:\Global PV and wind\ANS\etrans_cou_type_all_battery_mechanical_withoutPUE_pro2_8_2070.mat','etrans_cou_type')%
save('H:\Global PV and wind\ANS\etrans_REG_all_battery_mechanical_withoutPUE_pro2_8_2070.mat','etrans_REG')%
save('H:\Global PV and wind\ANS\etrans_REG_type_all_battery_mechanical_withoutPUE_pro2_8_2070.mat','etrans_REG_type')%

save('H:\Global PV and wind\ANS\powerall_reg_pro2_8_2070.mat','powerall_reg')% 不考虑用电效率的发电量, TWh/year

for i = 1:11
    power3(1,i) = etrans(i,i); % 自用实际发电量, TWh/year
    power3(2,i) = sum(etrans(i,:)) - etrans(i,i); % 他用实际发电量, TWh/year
    power3(3,i) = powerall_reg(i,1) - sum(etrans(i,:)); % 废弃电量, TWh/year
end
save('H:\Global PV and wind\ANS\power3_reg_pro2_8_2070.mat','power3')% 3*11，自用,他用和废弃实际发电量, TWh/year
save('H:\Global PV and wind\ANS\utilize_ratio2060_UHV_storage_inter_county_all_battery_mechanical_pro2_8_2070.mat','utilize_ratio2060_UHV_storage_inter')

for i = 1:11
    power3_2(1,i) = etrans1(i,i); % 自用实际发电量, TWh/year
    power3_2(2,i) = sum(etrans1(i,:)) - etrans1(i,i); % 他用实际发电量, TWh/year
end

for i = 1:192
    power3_cou(1,i) = etrans_cou(i,i); % 自用实际发电量, TWh/year
    power3_cou(2,i) = sum(etrans_cou(i,:)) - etrans_cou(i,i); % 他用实际发电量, TWh/year
    %     power3_cou(3,i) = e0_cou(i,1) - sum(etrans_cou(i,:)); % 废弃电量, TWh/year
    power3_cou_2(1,i) = etrans_cou1(i,i); % 自用实际发电量, TWh/year
    power3_cou_2(2,i) = sum(etrans_cou1(i,:)) - etrans_cou1(i,i); % 他用实际发电量, TWh/year
end
%%
...
for i = 1:size(distance_UHV_Station,1)
    j = i;
    linescapacity(i,j) = 0;
end
save('H:\Global PV and wind\ANS\linescapacity_all_battery_mechanical_pro2_8_2070.mat','linescapacity')%
save('H:\Global PV and wind\ANS\linescapacity_all_battery_mechanical_CN_pro2_8_2070.mat','linescapacity_CN')%


% 1000kV alternating current power
load('H:\Global PV and wind\Data\CPfix_AC.mat');  % lines
% AC 1000 kV: 1:transmission distance (km); 2:transmission capacity (MW)
for i = 1:size(distance_UHV_Station,1)
    for j = 1:size(distance_UHV_Station,1)
        CP_fix(i,j) = interp1(CPfix_AC(:,1), CPfix_AC(:,2),distance_UHV_Station(i,j),'linear'); % MW
    end
end
cost_line = 670785; % $/km
cost_sta = 41*10^3; % $ MW-1
cost_l_all1 = (linescapacity.*cost_sta+distance_UHV_Station.*cost_line.*ceil(linescapacity./CP_fix))/10^6; % million $

% ±1100kV direct  current power station
CP_fix = 12000; %MW
cost_line = 800383; % $/km
cost_sta = 92*10^3; % $ MW-1
cost_l_all2 = (linescapacity.*cost_sta+distance_UHV_Station.*cost_line.*ceil(linescapacity./CP_fix))/10^6; % million $

% ±800kV direct  current power station
CP_fix = 8000; %MW
cost_line = 732220; % $/km
cost_sta = 82*10^3; % $ MW-1
cost_l_all3 = (linescapacity.*cost_sta+distance_UHV_Station.*cost_line.*ceil(linescapacity./CP_fix))/10^6; % million $

Type_UHV = zeros(size(distance_UHV_Station,1),size(distance_UHV_Station,1));
for i = 1:size(distance_UHV_Station,1)
    for j = 1:size(distance_UHV_Station,1)
        if i~=j
            ccc = min([cost_l_all1(i,j); cost_l_all2(i,j); cost_l_all3(i,j)]);
            if ccc==cost_l_all1(i,j)
                Type_UHV(i,j) = 1;
            end
            if ccc==cost_l_all2(i,j)
                Type_UHV(i,j) = 2;
            end
            if ccc==cost_l_all3(i,j)
                Type_UHV(i,j) = 3;
            end
            cost_l_all(i,j) = ccc;
        end
...
% CN
load('H:\Global PV and wind\Data\tranmission_lines_CN.dat','-mat')
...
save('H:\Global PV and wind\ANS\CP_fix_8_2070.mat','CP_fix') % km
...
num_line_0 = lines(1:77,16);
num_line_0(num_line_0==1) = 0.1;
num_line_0(num_line_0==2) = 1;
num_line_0(num_line_0==3) = 1;
num_line_0(num_line_0==0.1) = 2;
cost_l_all_CN = (linescapacity_CN.*cost_sta+distance_CN.*cost_line.*max(ceil(linescapacity_CN./CP_fix)-num_line_0,0))/10^6; % million $

save('H:\Global PV and wind\ANS\cost_l_all_battery_mechanical_all_pro2_8_2070.mat','cost_l_all')% million $
save('H:\Global PV and wind\ANS\cost_l_all_battery_mechanical_all_CN_pro2_8_2070.mat','cost_l_all_CN')% million $
save('H:\Global PV and wind\ANS\Type_UHV_battery_mechanical_all_pro2_8_2070.mat','Type_UHV')% million $

% CP_trans_IX = reshape(sum(sum(linescapacity_plant,1),2),[10000,1]); % MW
% CP_trans_IX(10001:numpowerunit,1) = reshape(sum(sum(linescapacity_plant2,1),2),[numpowerunit-10000,1]); % MW

linescapacity1 = reshape(linescapacity,[size(distance_UHV_Station,1)*size(distance_UHV_Station,1),1]);
distance_UHV_Station1 = reshape(distance_UHV_Station,[size(distance_UHV_Station,1)*size(distance_UHV_Station,1),1]);
[BB,IXX] = sort(linescapacity1,'descend');
[distance_UHV_Station1(IXX) BB];
% 有很多线路运输很少的电，经济上不划算
save('H:\Global PV and wind\ANS\powergenerat_monhour_plant_county_battery_mechanical_all_pro2_8_2070.mat','powergenerat_monhour_plant')

save('H:\Global PV and wind\ANS\P_pro1_county_battery_mechanical_all_pro2_8_2070.mat','P_REG1')
save('H:\Global PV and wind\ANS\P_country1_county_battery_mechanical_all_pro2_8_2070.mat','P_country1')
save('H:\Global PV and wind\ANS\P_country2_county_battery_mechanical_all_pro2_8_2070.mat','P_country2')
save('H:\Global PV and wind\ANS\P_region2_county_battery_mechanical_all_pro2_8_2070.mat','P_region2')
save('H:\Global PV and wind\ANS\P_world_county_battery_mechanical_all_pro2_8_2070.mat','P_world')

save('H:\Global PV and wind\ANS\utilize_ratio2060_county_battery_mechanical_all_pro2_8_2070.mat','utilize_ratio2060')
save('H:\Global PV and wind\ANS\utilize_ratio2060_UHV_county_battery_mechanical_all_pro2_8_2070.mat','utilize_ratio2060_UHV')
save('H:\Global PV and wind\ANS\utilize_ratio2060_UHV_storage_county_battery_mechanical_all_pro2_8_2070.mat','utilize_ratio2060_UHV_storage')
save('H:\Global PV and wind\ANS\utilize_ratio2060_UHV_storage_reg_county_battery_mechanical_all_pro2_8_2070.mat','utilize_ratio2060_UHV_storage_reg')
save('H:\Global PV and wind\ANS\utilize_ratio2060_UHV_storage_inter_county_battery_mechanical_all_pro2_8_2070.mat','utilize_ratio2060_UHV_storage_inter')

save('H:\Global PV and wind\ANS\storage_max_plant_UHV_storage_county_battery_mechanical_all_pro2_8_2070.mat','storage_max_plant_UHV_storage')   % TWh/h
save('H:\Global PV and wind\ANS\storage_year_plant_UHV_storage_county_battery_mechanical_all_pro2_8_2070.mat','storage_year_plant_UHV_storage')  % TWh/year
save('H:\Global PV and wind\ANS\storage_max_plant_UHV_storage_reg_county_battery_mechanical_all_pro2_8_2070.mat','storage_max_plant_UHV_storage_reg')   % TWh/h
save('H:\Global PV and wind\ANS\storage_year_plant_UHV_storage_reg_county_battery_mechanical_all_pro2_8_2070.mat','storage_year_plant_UHV_storage_reg')  % TWh/year
save('H:\Global PV and wind\ANS\storage_max_plant_UHV_storage_inter_county_battery_mechanical_all_pro2_8_2070.mat','storage_max_plant_UHV_storage_inter')   % TWh/h
save('H:\Global PV and wind\ANS\storage_year_plant_UHV_storage_inter_county_battery_mechanical_all_pro2_8_2070.mat','storage_inter_year_plant_UHV_storage')  % TWh/year

%%
linescapacity_plant_all = zeros(size(distance_UHV_Station,1),size(distance_UHV_Station,1));
for n = 1:1:nnnn+1
    filename = strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str(n),'_battery_mechanical_pro_8_2070.mat');
    load(filename);
    linescapacity_plant_all = linescapacity_plant_all+sum(linescapacity_plant,3);
    CP_trans_IX((n-1)*1000+1:n*1000,1) = reshape(sum(sum(linescapacity_plant,1),2),[1000,1]); % MW
end
CP_trans_IX(numpowerunit+1:end)=[];
CP_trans_IX = CP_trans_IX+sum(linescapacity_plant_CN)'; % MW

cost_trans_IX = zeros((nnnn+1)*1000,1);
cost_trans_IX_cou = zeros((nnnn+1)*1000,192);
for n = 1:1:nnnn+1
    filename = strcat('H:\Global PV and wind\ANS\linescapacity_plant',num2str(n),'_battery_mechanical_pro_8_2070.mat');
    load(filename);
    ...
    for i = 1:size(distance_UHV_Station,1)
        for j = 1:size(distance_UHV_Station,1)
            if cost_l_all(i,j)~=0
                a = reshape(cost_l_all(i,j)*(linescapacity_plant(i,j,:)./(sum(linescapacity_plant_all(i,j,:)))),[1000,1]);
                a(find(isnan(a)==1))=0;
                cost_trans_IX((n-1)*1000+1:n*1000,1) = cost_trans_IX((n-1)*1000+1:n*1000,1) + a;  % million $
                cost_trans_IX_cou((n-1)*1000+1:n*1000,floor(UHV_Station_country(j,4))) = cost_trans_IX_cou((n-1)*1000+1:n*1000,floor(UHV_Station_country(j,4))) + a;  % million $
            end
        end
%         i
    end
...
cost_trans_IX(numpowerunit+1:end)=[];
cost_trans_IX(find(isnan(cost_trans_IX)==1))=0;

cost_trans_IX_cou(numpowerunit+1:end,:)=[];
cost_trans_IX_cou(find(isnan(cost_trans_IX_cou)==1))=0;

for i = 1:77
    cost_l_plant_CN(i,:) = cost_l_all_CN(i,1)*(linescapacity_plant_CN(i,:)./sum(linescapacity_plant_CN(i,:))); % million $
end
cost_l_plant_CN(find(isnan(cost_l_plant_CN)==1))=0;
cost_trans_IX_CN(:,1) = sum(cost_l_plant_CN)';% million $

cost_trans_IX = cost_trans_IX+cost_trans_IX_CN; % million $
cost_trans_IX_cou(:,35) = cost_trans_IX_cou(:,35) + cost_trans_IX_CN;
save('H:\Global PV and wind\ANS\cost_trans_IX_battery_mechanical_all_pro2_8_2070.mat','cost_trans_IX')% million $
save('H:\Global PV and wind\ANS\CP_trans_IX_battery_mechanical_all_pro2_8_2070.mat','CP_trans_IX')% million $
save('H:\Global PV and wind\ANS\cost_trans_IX_cou2_8_2070.mat','cost_trans_IX_cou')% million $
% 第j列表示从这个电厂到j国所需的UHV cost