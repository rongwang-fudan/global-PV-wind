...
save('H:\Global PV and wind\ANS\EF_2040_2040.mat','EF','-v7.3')  % t CO2/kWh
save('H:\Global PV and wind\ANS\unitmin_2040_2040.mat','unitmin','-v7.3')  % t CO2/kWh
...
Ph_PVwind5_ef_cou_p2040 = Ph_PVwind5_ef_cou_p(:,:,4);
r = sum(Ph_PVwind5_ef_cou_p2040,2)./phhall_all2;
phhall_all2 = sum(Ph_PVwind5_ef_cou_p(:,:,4),2);
...
save('H:\Global PV and wind\ANS\B_utilize_trans_storage_nodiffuse_2040_2040.mat','B_utilize_trans_storage_nodiffuse','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\B_utilize_trans_storage_2040_2040.mat','B_utilize_trans_storage','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\CO2_year_utilize_trans_storage_2040_2040.mat','CO2_year_utilize_trans_storage','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\phhall_all2_2040_2040.mat','phhall_all2','-v7.3')  % TWh/year
...
for i = 1:192
    ...
    [m,n]=find(powerunit_country_IX_IX==i & optpowerunit_IX_IX(:,35)==1 & unitmin==3 & sum(Owncountry_ID2,2)~=0);% find power plant situated in ocuntry i and without mineral trade
    r_min = r_mineral_30sa(i,1);
    r_power_coop(m,1) = r_power_coop(m,1)+r_min*r_etrans_cou1_num(m,i,i);
    power_mineral(1,1) = power_mineral(1,1)+r_min*sum(etrans_cou1_num(m,i,i)); % total power without mineral and power trade
    power_mineral_cou(i,2) = power_mineral_cou(i,2)+r_min*sum(etrans_cou1_num(m,i,i)); % total power without mineral and power trade
    inv_cou(i,1) = inv_cou(i,1)+r_min*sum(Inv_nodiffuse(m).*r_etrans_cou1_num(m,i,i));
    ...
    [m,n]=find(powerunit_country_IX_IX==i & optpowerunit_IX_IX(:,35)==1 & unitmin==4 & sum(Owncountry_ID2,2)~=0);% find power plant situated in ocuntry i and without mineral trade
    r_min = r_mineral_30sb(i,1);
    r_power_coop(m,1) = r_power_coop(m,1)+r_min*r_etrans_cou1_num(m,i,i);
    power_mineral(1,1) = power_mineral(1,1)+r_min*sum(etrans_cou1_num(m,i,i)); % total power without mineral and power trade
    power_mineral_cou(i,2) = power_mineral_cou(i,2)+r_min*sum(etrans_cou1_num(m,i,i)); % total power without mineral and power trade
    inv_cou(i,1) = inv_cou(i,1)+r_min*sum(Inv_nodiffuse(m).*r_etrans_cou1_num(m,i,i));
...
save('H:\Global PV and wind\ANS\power_mineral_cou2040_2040.mat','power_mineral_cou','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\inv_total_cou_PW_noconn_2040_2040.mat','inv_total_cou','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\inv_cou_PW_noconn_2040_2040.mat','inv_cou','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\asw.mat','asw','-v7.3')  % million $
nn_c = nn_c+1;
...
save('H:\Global PV and wind\ANS\r_power_coop_notech_nopower_nomineraltrans2040.mat','r_power_coop_notech_nopower_nomineraltrans2040','-v7.3')  % TWh/year
...
for i = 1:192
    for j = 1:192
        if i~=j
            ...
            r_min = r_mineral_20sa(i,1);
            power_mineral_cou(j,2) = power_mineral_cou(j,2)+r_min*sum(etrans_cou1_num(m,j,j)); 
            ...
            [m,n]=find(powerunit_country_IX_IX==j & optpowerunit_IX_IX(:,35)==1 & unitmin==2 & sum(Owncountry_ID2,2)~=0 & B_utilize_trans_storage_nodiffuse<=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            r_min = r_mineral_20sb(i,1);
            ...
            [m,n]=find(powerunit_country_IX_IX==j & optpowerunit_IX_IX(:,35)==1 & unitmin==3 & sum(Owncountry_ID2,2)~=0 & B_utilize_trans_storage_nodiffuse<=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            r_min = r_mineral_30sa(i,1);
            power_mineral(nn_c,1) = power_mineral(nn_c,1)+r_min*sum(etrans_cou1_num(m,j,j)); % total power without mineral and power trade
            power_mineral_cou(j,2) = power_mineral_cou(j,2)+r_min*sum(etrans_cou1_num(m,j,j)); 
            plant_use(m,1) = plant_use(m,1)+r_min;
            ...
            [m,n]=find(powerunit_country_IX_IX==j & optpowerunit_IX_IX(:,35)==1 & unitmin==4 & sum(Owncountry_ID2,2)~=0 & B_utilize_trans_storage_nodiffuse<=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            r_min = r_mineral_30sb(i,1);
            power_mineral(nn_c,1) = power_mineral(nn_c,1)+r_min*sum(etrans_cou1_num(m,j,j)); % total power without mineral and power trade
            power_mineral_cou(j,2) = power_mineral_cou(j,2)+r_min*sum(etrans_cou1_num(m,j,j)); 
            plant_use(m,1) = plant_use(m,1)+r_min;
            ...
            power_mineral_powertrans_cou(j,2) = power_mineral_powertrans_cou(j,2)+r_min*sum(etrans_cou1_num(m1,j,j))+sum(etrans_cou1_num(m,i,j));
            ...
            [m1,n]=find(powerunit_country_IX_IX==j & optpowerunit_IX_IX(:,35)==1 & unitmin==2 & sum(Owncountry_ID2,2)~=0 & B_utilize_trans_storage_nodiffuse <=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            [m,n]=find(powerunit_country_IX_IX==i & optpowerunit_IX_IX(:,35)==1 & unitmin==2 & B_utilize_trans_storage_nodiffuse <=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            r_min = r_mineral_20sb(i,1);
            ...
            [m1,n]=find(powerunit_country_IX_IX==j & optpowerunit_IX_IX(:,35)==1 & unitmin==3 & sum(Owncountry_ID2,2)~=0 & B_utilize_trans_storage_nodiffuse <=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            [m,n]=find(powerunit_country_IX_IX==i & optpowerunit_IX_IX(:,35)==1 & unitmin==3 & B_utilize_trans_storage_nodiffuse <=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            r_min = r_mineral_30sa(i,1);
            power_mineral_powertrans(nn_c,1) = power_mineral_powertrans(nn_c,1)+r_min*sum(etrans_cou1_num(m1,j,j))+sum(etrans_cou1_num(m,i,j));
            power_mineral_powertrans_cou(j,2) = power_mineral_powertrans_cou(j,2)+r_min*sum(etrans_cou1_num(m1,j,j))+sum(etrans_cou1_num(m,i,j));
            Inv_pvwind_mineral_powertrans(asd,1) = Inv_pvwind_mineral_powertrans(asd,1)+sum(Inv_nodiffuse(m).*r_etrans_cou1_num(m,i,j))+sum(r_min.*Inv_nodiffuse(m1).*r_etrans_cou1_num(m1,j,j));
            ...
            [m1,n]=find(powerunit_country_IX_IX==j & optpowerunit_IX_IX(:,35)==1 & unitmin==4 & sum(Owncountry_ID2,2)~=0 & B_utilize_trans_storage_nodiffuse <=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            [m,n]=find(powerunit_country_IX_IX==i & optpowerunit_IX_IX(:,35)==1 & unitmin==4 & B_utilize_trans_storage_nodiffuse <=MAC_CCS);% find power plant situated in ocuntry i and without mineral trade
            r_min = r_mineral_30sb(i,1);
            power_mineral_powertrans(nn_c,1) = power_mineral_powertrans(nn_c,1)+r_min*sum(etrans_cou1_num(m1,j,j))+sum(etrans_cou1_num(m,i,j));
            power_mineral_powertrans_cou(j,2) = power_mineral_powertrans_cou(j,2)+r_min*sum(etrans_cou1_num(m1,j,j))+sum(etrans_cou1_num(m,i,j));
            Inv_pvwind_mineral_powertrans(asd,1) = Inv_pvwind_mineral_powertrans(asd,1)+sum(Inv_nodiffuse(m).*r_etrans_cou1_num(m,i,j))+sum(r_min.*Inv_nodiffuse(m1).*r_etrans_cou1_num(m1,j,j));
...
save('H:\Global PV and wind\ANS\power_mineraltrans_cou2040_2040.mat','power_mineral_cou','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\power_mineral_powertrans_cou2040_2040.mat','power_mineral_powertrans_cou','-v7.3')  % TWh/year
...
save('H:\Global PV and wind\ANS\r_power_coop_notech_nopowertrans_2040.mat','r_power_coop_notech_nopowertrans2040','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\power_mineral_2040.mat','power_mineral2040','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\Inv_pvwind_mineral_2040.mat','Inv_pvwind_mineral2040','-v7.3')  %
save('H:\Global PV and wind\ANS\Inv_pvwind_mineral_2040_total.mat','Inv_pvwind_mineral2040_total','-v7.3')  %
...
save('H:\Global PV and wind\ANS\r_power_coop_notech_2040.mat','r_power_coop_notech2040','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\power_mineral_powertrans_2040.mat','power_mineral_powertrans2040','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\Inv_pvwind_mineral_powertrans_2040.mat','Inv_pvwind_mineral_powertrans2040','-v7.3')  %
save('H:\Global PV and wind\ANS\Inv_pvwind_mineral_powertrans_2040_total.mat','Inv_pvwind_mineral_powertrans2040_total','-v7.3')  %

[B,IX2] = sort(power_mineral_powertrans(2:nn_c-1),'descend');
index_ij_IX = index_ij(IX2+1,1:2);
save('H:\Global PV and wind\ANS\index_ij_2040.mat','index_ij')  % TWh/year
save('H:\Global PV and wind\ANS\index_ij_IX_2040.mat','index_ij_IX')  %
...
load('H:\Global PV and wind\ANS\Inn_2040.mat')
Inn_2055_2040_ixx = flip(Inn_2040(:,1:2));
Inn_2055_2040_num = Inn_2040(:,3:size(Inn_2040,2));
...
save('H:\Global PV and wind\ANS\index_ij_IX_2040xz.mat','index_ij_IX')  %
...
save('H:\Global PV and wind\ANS\power_mineral_powertrans_tech_cou2040_2040.mat','power_mineral_powertrans_tech_cou','-v7.3')  % TWh/year
...
save('H:\Global PV and wind\ANS\r_power_coop_2040.mat','r_power_coop2040','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\power_mineral_powertrans_tech_2040.mat','power_mineral_powertrans_tech2040','-v7.3')  % TWh/year
save('H:\Global PV and wind\ANS\Inv_pvwind_mineral_powertrans_tech_2040.mat','Inv_pvwind_mineral_powertrans_tech2040','-v7.3')  %
