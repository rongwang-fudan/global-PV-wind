...
load('H:\Global PV and wind\ANS\Inv_CCSall_dis2a_8_2040.mat')  %Inv_CCSall_dis2a，单个关系建立导致的净成本变化
load('H:\Global PV and wind\ANS\Inv_othersall_dis2a_8_2040.mat')  %Inv_othersall_dis2a，单个关系建立导致的净成本变化
load('H:\Global PV and wind\ANS\Inv_PWall_dis2a_8_2040.mat')  %Inv_PWall_dis2a，单个关系建立导致的净成本变化
load('H:\Global PV and wind\ANS\Inv_CCS0_dis2a_8_2040.mat')  %Inv_CCS0_dis2a,没有PV和wind时的净投资
load('H:\Global PV and wind\ANS\Inv_others_c0_dis2a_8_2040.mat')  %Inv_others_c0_dis2a,没有PV和wind时的净投资
...
% end
Inv0_cou2040 = a2;% 没有PV和wind时各国各年的CCS和other renewable的投资
...
% Inv0_coudif = Inv0_cou2040*rrr/10^6*10;
% Inv_dif_noconnect_cou = Inv_dif2040_noconnect_cou*rrr/10^6*10;
% A = Inv_dif2040(2:end)*rrr;
% A(A>0)=0;
% 
% Cost_reduct_cou = zeros(192,1);
% for i = 1:192
%     [m,n]=find(index_ij_s(:,2)==i);
%     Cost_reduct_cou(i,1) = sum(A(m))/10^6*10;
% end
% NetInv = Inv0_coudif+Inv_dif_noconnect_cou+Cost_reduct_cou;
% sum(NetInv)
Inv0_coudif = Inv0_cou2040*rrr/10^6*5;% 没有PV和wind时各国各年的CCS和other renewable的discounted to 2019之后的投资
Inv_dif_noconnect_cou = Inv_dif2040_noconnect_cou*rrr/10^6*5;% 没有国际合作时的CCS,other renewable和PV&wind的的投资的变化
A = Inv_dif2040(2:end,:)*rrr;% 逐个建立合作关系导致的净成本变化
A(A>0)=0;
...
NetInv = Inv0_coudif+Inv_dif_noconnect_cou+Cost_reduct_cou;
sum(NetInv)
save('H:\Global PV and wind\ANS\NetInv_nodiscounted_8_2040peryear.mat','NetInv'); % trillion discounted net cost, trillion 2020$



a = sum(NetInv);
for i = 1:5
    a2222(i) = sum(a(i:i+1))/2;
end
