...
clear initialcost_ratio_country2

learnprice_region2_2 = ones(192,6,11);
learnprice_region_nodiffuse = ones(192,6,11);
learnprice_region1(:,1) = initialcost_ratio_country(:,1);
learnprice_region1(:,2) = initialcost_ratio_country(:,1);
learnprice_region1(:,3) = initialcost_ratio_country(:,2);
learnprice_region1(:,4) = initialcost_ratio_country(:,2);
learnprice_region1(:,5) = initialcost_ratio_country(:,3);
learnprice_region1(:,6) = initialcost_ratio_country(:,3);
learnprice_region2_2(:,:,1) = learnprice_region1;
...
for i = 2:1:11
    ...
    learnprice_region1(:,1) = learnprice_region(:,1).*initialcost_ratio_country(:,1);
    learnprice_region1(:,2) = learnprice_region(:,2).*initialcost_ratio_country(:,1);
    learnprice_region1(:,3) = learnprice_region(:,3).*initialcost_ratio_country(:,2);
    learnprice_region1(:,4) = learnprice_region(:,4).*initialcost_ratio_country(:,2);
    learnprice_region1(:,5) = learnprice_region(:,5).*initialcost_ratio_country(:,3);
    learnprice_region1(:,6) = learnprice_region(:,6).*initialcost_ratio_country(:,3);
    learnprice_region2_2(:,:,i) = learnprice_region1; % 各国价格
    learnprice_region_nodiffuse(:,:,i) = learnprice_region;
end
clear learnprice_region

learnprice_region = zeros(192,6,11);
for countrya = 1:192
    for i = 1:192
        initial = [initialcost_ratio_country(i,1);initialcost_ratio_country(i,1);initialcost_ratio_country(i,2);initialcost_ratio_country(i,2);initialcost_ratio_country(i,3);initialcost_ratio_country(i,3)];
        learnprice_region(i,:,:) = reshape(learnprice_region2_2(countrya,:,:),[6,11])./repmat(initial,1,11);
    end
    learnprice_region(:,:,1) = 1;
    ...
    for region = 1:1:192
        module_price_offshore = initialcost_ratio_country_off(region).*1.008; % USD/W
        for nnna = 1:10
    ...
    B_m(m,1) = Inf;
    B_utilize_trans_storage_192(:,countrya) = B_utilize_trans_storage;
    B_m_192(:,countrya) = B_m;
    countrya
    clearvars -except B_m_192 initialcost_ratio_country_off r_fixednomodule_hardwarecost_ons learnprice_region_nodiffuse B_utilize_trans_storage_192 learnprice_region2_2 initialcost_ratio_country r_module_hardwarecost_pv r_module_hardwarecost_ons
end
save('H:\Global PV and wind\ANS\B_UHV_STO_INT_county_all_withUHVcost_pro2_192countrya_2040_8.mat','B_utilize_trans_storage_192', '-v7.3')  % billion kWh / year
save('H:\Global PV and wind\ANS\B_m_192countrya_2040_8.mat','B_m_192', '-v7.3')  % 