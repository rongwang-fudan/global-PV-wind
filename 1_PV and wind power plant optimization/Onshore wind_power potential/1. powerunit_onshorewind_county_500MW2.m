...
% load('H:\Global PV and wind\Data\Slope_world_xz.mat');
load('H:\Global PV and wind\Data\Slope_world.mat');
% Ground slope (%)
load('H:\Global PV and wind\Data\CF_mean.mat') %单位 km2
CF_mean = CF;
load('H:\Global PV and wind\Data\CF_mean_max.mat') %单位 km2

load('H:\Global PV and wind\Data\nature_reserve_wildlife120_xz.mat'); %
load('H:\Global PV and wind\Data\DEM_world_xz.mat'); %
...
for coun = 1:34 %max(max(GADM_country120)) %1:43% 45:59 %61:
    ...
    if max(n)-min(n)<4000
        if (~isempty(m))
            ...
            filename = strcat('H:\Global PV and wind\Code\1_PV and wind power plant optimization\Onshore wind_power potential\ANS\powerunit_',num2str(coun),'_sameC.mat');
            save(filename, 'powerunit','-v7.3');
            
            filename = strcat('H:\Global PV and wind\Code\1_PV and wind power plant optimization\Onshore wind_power potential\ANS\unitid_',num2str(coun),'_sameC.mat');
            save(filename, 'unitid','-v7.3');
            
            filename = strcat('H:\Global PV and wind\Code\1_PV and wind power plant optimization\Onshore wind_power potential\ANS\unitid_all_',num2str(coun),'_sameC.mat');
            save(filename, 'unitid_all','-v7.3');
            
            filename = strcat('H:\Global PV and wind\Code\1_PV and wind power plant optimization\Onshore wind_power potential\ANS\unitid_cy_',num2str(coun),'_sameC.mat');
...
save('H:\Global PV and wind\Code\1_PV and wind power plant optimization\Onshore wind_power potential\ANS\position_county_onshorewind_500MW_sameC.dat','position_county');
