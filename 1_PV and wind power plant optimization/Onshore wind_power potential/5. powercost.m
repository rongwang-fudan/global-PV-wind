...
% ENERGY Production 2
load('H:\global-PV-wind\Data\CP_wind_120_all_2.mat'); % CP_PV_120_all(4800x1950)
...
load('H:\global-PV-wind\Code\1_PV and wind power plant optimization\Onshore wind_power potential\ANS\suitableland_onshorewind.mat')
% suitablity1 = zeros(21600,10800);
% suitablity1(35*120+1:75*120,(180+73)*30+1:(180+138)*30) = suitablity;
% clear suitablity
% [m,n]=find(suitablity1~=0);
% suitableland_onshorewind(sub2ind(size(suitableland_onshorewind), m, n))= suitablity1(sub2ind(size(suitablity1), m, n));
suitablity = suitableland_onshorewind;
...
for coun = 1:34
    ...
    for ii=1:size(powerunit1,1)
        %     i=i+1;
        nummmm = powerunit(mp(ii),4);
        cy = powerunit(mp(ii),6);
        ...
        powercy=E_120(cy,coun)/topon(nummmm) ; % GWh/day/county,topon(nummmm)
        isnottransmit=0;
        ...
        for j=1:zmax
            ...
            if size(idx2,1)>0
                for k=1:size(idx222,1)
                    ...
                    costs(id,12)=costs(id,12)+costland_PV120(x,y)/1e6*suitablity(x,y)*0.04; % cost of purchasing the land million RMB
                end
                for k=1:size(idx2,1)
...
%                         costs(id,4)=costs(id,4)+Carbon_density12030(x,y)*gridarea(x,y)*1000/6.67*suitablity(x,y)*0.02; % land-use change emission t C/ha -> ton C
                        costs(id,4)=costs(id,4)+Carbon_density12030(x,y)*gridarea(x,y)*100*suitablity(x,y)*0.02; % land-use change emission t C/ha -> ton C
                        costs(id,5)=costs(id,5)+land_sink(x,y)*gridarea(x,y)*suitablity(x,y)*0.02; % land carbon sink gC/m2/yr -> ton C/yr
                    end
        ...
        for j=1:zmax
            ...
            if size(idx2,1)>0
                ...
                for k=1:size(idx2,1)
                    ...
                    % For wind energy, there is no land-use change emission or land carbon sink change
                    if LC_CN(x,y)<=10 || LC_CN(x,y)==12 || LC_CN(x,y)==14
                        costs(id,4)=costs(id,4)+Carbon_density12030(x,y)*gridarea2(x-35*120,y-(180+73)*30)*100*suitablity(x,y)*0.02; % land-use change emission t C/ha -> ton C
                        costs(id,5)=costs(id,5)+land_sink(x,y)*gridarea2(x-35*120,y-(180+73)*30)*suitablity(x,y)*0.02; % land carbon sink gC/m2/yr -> ton C/yr
                    end
                end
            end
        end
...
save('H:\global-PV-wind\Code\1_PV and wind power plant optimization\Onshore wind_power potential\ANS\costunits_500MW_2_all.dat','costs','-v7.3');
% clear
% toc
