tic
clear;
load('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\position_county_PV_sameC.dat','-mat');
% 最小行，最大行，最小列、最大列，第5列如果是2代表左边是东半球，右边是西半球（与原始数据左右调换）
load('H:\global-PV-wind\Data\GADM_country120_xz2.mat') 


powerunit_w = [];
unitid_w = zeros(21600,10800);
unitid_all_w = zeros(21600,10800);
unitid_cy_w = zeros(21600,10800);
num=0;
for iii = 1:max(max(GADM_country120))
    coun = iii;

    load(strcat('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\powerunit_',num2str(coun),'_sameC.mat'));
    load(strcat('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\unitid_',num2str(coun),'_sameC.mat'));
    load(strcat('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\unitid_all_',num2str(coun),'_sameC.mat'));
    load(strcat('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\unitid_cy_',num2str(coun),'_sameC.mat'));
    if (~isempty(powerunit))
    unitid_w11 = zeros(21600,10800);
    unitid_all_w11 = zeros(21600,10800);
    unitid_pro_w11 = zeros(21600,10800);
    if position_county(coun,5)~=2
        powerunit_w1(:,1) = powerunit(:,1)+position_county(coun,1)-1;
        powerunit_w1(:,2) = powerunit(:,2)+position_county(coun,3)-1;
        powerunit_w1(:,3:6) = powerunit(:,3:6);
        unitid_w11(position_county(coun,1):position_county(coun,2),position_county(coun,3):position_county(coun,4))=unitid;
        unitid_all_w11(position_county(coun,1):position_county(coun,2),position_county(coun,3):position_county(coun,4))=unitid_all;
        unitid_pro_w11(position_county(coun,1):position_county(coun,2),position_county(coun,3):position_county(coun,4))=unitid_cy;
    end
    
    if position_county(coun,5)==2
        unitid_w111 = zeros(21600,10800);
        unitid_all_w111 = zeros(21600,10800);
        unitid_pro_w111 = zeros(21600,10800);
        powerunit_w1(:,1) = powerunit(:,1)+position_county(coun,1)-1;
        powerunit_w1(:,2) = powerunit(:,2)+position_county(coun,3)-1;
        powerunit_w1(:,3:6) = powerunit(:,3:6);
        for i = 1:size(powerunit_w1,1) 
            if powerunit_w1(i,2)<=5400
                powerunit_w1(i,2) = powerunit_w1(i,2)+5400;
            else if powerunit_w1(i,2)>5400
                    powerunit_w1(i,2) = powerunit_w1(i,2)-5400;
                end
            end
        end
        unitid_w111(position_county(coun,1):position_county(coun,2),position_county(coun,3):position_county(coun,4))=unitid;
        unitid_all_w111(position_county(coun,1):position_county(coun,2),position_county(coun,3):position_county(coun,4))=unitid_all;
        unitid_pro_w111(position_county(coun,1):position_county(coun,2),position_county(coun,3):position_county(coun,4))=unitid_cy;

        unitid_w11(:,1:5400) = unitid_w111(:,5401:10800);
        unitid_w11(:,5401:10800) = unitid_w111(:,1:5400);
        unitid_all_w11(:,1:5400) = unitid_all_w111(:,5401:10800);
        unitid_all_w11(:,5401:10800) = unitid_all_w111(:,1:5400);
        unitid_pro_w11(:,1:5400) = unitid_pro_w111(:,5401:10800);
        unitid_pro_w11(:,5401:10800) = unitid_pro_w111(:,1:5400);
    end
    
    powerunit_w = [powerunit_w;powerunit_w1];
    clear powerunit_w1
    [m,n]=find(unitid_w11~=0);
    unitid_w(sub2ind(size(unitid_w), m, n))= unitid_w11(sub2ind(size(unitid_w11), m, n))+num;
    [m,n]=find(unitid_all_w11~=0);
    unitid_all_w(sub2ind(size(unitid_all_w), m, n))= unitid_all_w11(sub2ind(size(unitid_all_w11), m, n))+num;
    [m,n]=find(unitid_pro_w11~=0);
    unitid_cy_w(sub2ind(size(unitid_cy_w), m, n))= unitid_pro_w11(sub2ind(size(unitid_pro_w11), m, n));
    num = num+max(max(unitid_w11));
    end
    coun
end
unitid_cy_w = floor(unitid_cy_w);


save('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\unitid_w_pv_county.dat','unitid_w'); % 电厂id，实际面积
save('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\unitid_all_w_pv_county.dat','unitid_all_w'); % 电厂id，电厂范围内所有面积
save('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\unitid_cy_w_pv_county.dat','unitid_cy_w'); % cy,每个国家内部排序
save('H:\global-PV-wind\Code\1_PV and wind power plant optimization\PV_power potential\ANS\powerunit_w_pv_county.dat','powerunit_w');
